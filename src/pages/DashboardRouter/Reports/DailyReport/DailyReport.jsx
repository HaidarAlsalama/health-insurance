import { useDailyReport } from "API/report";
...
export default function DailyReport() {
  ...
  } = useDailyReport(startDate, endDate);
  return (
    <div className="bg-gray-50 rounded-md h-full p-4">
      <div className="flex gap-4 flex-wrap justify-center">
        {" "}
        <button
          className="btn btn-success !p-1 !text-sm !px-2 w-40"
          disabled={!reportIsSuccess}
          onClick={() =>
            window.open(
              `${config.baseUrl1}/EXCEL/GenerateReport?startDate=${startDate}&endDate=${endDate}`,
        ...
        <button
          ...
          onClick={() =>
            window.open(
              `${config.baseUrl1}/EXCEL/GenerateDetailedExcelReport`,
              "_blank" // لفتح الرابط في علامة تبويب جديدة
            )
          }
        >
      ...
      <ContainerInputs className="bg-white dark:bg-gray-900" title={"الفلاتر "}>
        <InputField
          title={"من تاريخ"}
          value={startDate}
